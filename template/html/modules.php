...
function getPositionAutospanWidth($position)
{
	...
	if ($robModules)
	{
		foreach ($robModules as $robModule)
		{
			...
			if ($bootstrapSize != 0)
			{
				$availableColumns -= $bootstrapSize;
				$autospanModules--;
			}
		}
	}
...
/*
 * Wright (divs and font header tags)
 * With the new advanced parameter it does the same as the html5 chrome
 */
function modChrome_whtml($module, &$params, &$attribs)
{
...
function modChrome_wrightflexgrid($module, &$params, &$attribs)
{
	...
	if ($modulenumbera[$attribs['name']] == 1)
	{
		...
		// Dor 5 modules with span2 first and last modules will have 3 columns width
